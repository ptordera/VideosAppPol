...
@section('content')
    <div class="container py-5">
        <h1 class="mb-4">Editar Vídeo: {{ $video->title }}</h1>
        ...
        <x-card>
            <form action="{{ route('videos.update', $video->id) }}" method="POST" data-qa="video-edit-form">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="published_at" id="published_at" class="form-control" value="{{ old('published_at', \Carbon\Carbon::parse($video->published_at)->format('Y-m-d')) }}" required>
                </div>

                <div class="mb-4">
                    <label for="previous" class="form-label">Vídeo anterior (opcional)</label>
                    <input type="text" name="previous" id="previous" class="form-control" value="{{ old('previous', $video->previous) }}">
                </div>

                <div class="mb-4">
                    <label for="next" class="form-label">Vídeo següent (opcional)</label>
                    <input type="text" name="next" id="next" class="form-control" value="{{ old('next', $video->next) }}">
                </div>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <x-button type="primary">Actualitzar Vídeo</x-button>
                </div>
            </form>
        </x-card>
    </div>
